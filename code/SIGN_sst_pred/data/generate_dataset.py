...
import argparse
from model.utils import get_edgeindex_4_edges, sample_observed_dynamics
...
class DatasetGenerator:
    ...
    def plot_single_sample(self, solutions, variable_names=None):
        ...
        if num_nodes > self.args.plot_node_num:
            selected_nodes = torch.randperm(num_nodes)[:self.args.plot_node_num]
        else:
    ...
    def generate_batch_sample(self):
        ...
        for _ in tqdm(range(self.args.sample_num)):
            ...
            observed_dynamics, observed_edge_index, observed_nodes = sample_observed_dynamics(solution, edge_index, self.args.ob_node_rate)

            data = Data(
                x=observed_dynamics.permute(1,0,2),
                edge_index=observed_edge_index,
                t=timestamps,
                para = self.para
            )
...
def get_args():
    ...
    parser.add_argument('--save_path', type=str, default='./SIGN_data', help='Path to save the generated data')
    parser.add_argument('--ob_node_rate', type=float, default=0.8, help='observased node rate')
    ...
    parser.add_argument('--model_name', type=str, choices= ['SIS', 'Kuramoto', 'Gene'], default='SIS', help='Name of the model for saving data')
    # SIS: 0.5, Kuramoto: 10, Gene: 1
    parser.add_argument('--init_scale', type=float, default=10, help='Scale factor for initial state values')
