...
class GSICell(MessagePassing):
    def __init__(self, args):
        ...
        self.wf_2 = nn.Parameter(
            torch.cat((
                0.9 + 0.1 * torch.rand(self.num_func_lib, 1),  # 生成 U(0.9, 0.99)
                0.1 * torch.rand(self.num_func_lib, 1)  # 生成 U(0.01, 0.1)
            )), 
            requires_grad=True
        )
        ...
        self.wc_2 = nn.Parameter(
            torch.cat((
                0.9 + 0.1 * torch.rand(self.num_coupled_fun_lib, 1),  # 生成 U(0.9, 0.99)
                0.1 * torch.rand(self.num_coupled_fun_lib, 1)  # 生成 U(0.01, 0.1)
            )), 
            requires_grad=True
        )
        # 边属性处理
...
class CGSIDecoder(nn.Module):
    ...
    def forward(self, t, batchs, c_mask=None, f_mask=None):
        ...
        # 2. 时间窗口分割
        all_preds = []
        total_steps = batchs.x.shape[1]  # 获取总时间步数
