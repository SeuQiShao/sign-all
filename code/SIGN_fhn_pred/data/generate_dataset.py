...
import argparse
from model.utils import get_edgeindex_4_edges, partition_graph_pyg
...
class DatasetGenerator:
        ...
        if network_type == 'random':
            ...
            try:
                ...
                if 'bn-human' in self.args.file_path:
                    sub_edge_indices, sub_num_nodes = partition_graph_pyg(edge_index, n, num_parts=4)
                    edge_index = sub_edge_indices[-1]
                    n = sub_num_nodes[-1]
    ...
    def initialize_model(self, edge_index):
        ...
        if self.args.model_name == 'FHN':
            self.model = FitzHughDynamics(edge_index).to(self.args.device)
    ...
    def generate_batch_sample(self):
        ...
        for _ in tqdm(range(self.args.sample_num)):
            ...
            x0 = torch.rand((self.node_num, self.args.init_dim), device=self.args.device) * torch.tensor(self.args.init_scale_range, device=self.args.device) + torch.tensor(self.args.init_scale_bias, device=self.args.device)
...
def get_args():
    ...
    parser.add_argument('--sample_interval', type=float, default=0.02, help='Total time for ODE integration')
    parser.add_argument('--num_points', type=int, default=10000, help='Number of time points in the time series')
    ...
    parser.add_argument('--save_path', type=str, default='/data/scratch/acw802/SIGN_lasso_hd_pred', help='Path to save the generated data')
    ...
    parser.add_argument('--network', type=str, choices=['random', 'power_law', 'small_world', 'from_file'], default='from_file', help='Type of graph topology to use')
    ...
    parser.add_argument('--file_path', type=str, default='/data/scratch/acw802/networks/bn-human-BNU_1_0025890_session_178k/bn-human-BNU_1_0025890_session_1.edges', help='file path')
    ...
    parser.add_argument('--plot_node_num', type=int, default=1, help='Number of nodes to plot.')
    ...
    parser.add_argument('--init_scale_range', type=float, default=[-1, -1, 1], help='Scale factor for initial state values')
    parser.add_argument('--init_scale_bias', type=float, default=[-0.8, -8, 2.8], help='Scale factor for initial state values')
