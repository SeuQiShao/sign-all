...
def sample_gumbel(shape, eps=1e-10):
    """
    Adapted from the PyTorch Gumbel-Softmax implementation:
...
def distribute_over_GPUs(args, model, num_GPU=None):
    ...
    if args.device.type != "cpu":
        ...
        assert num_GPU <= torch.cuda.device_count(), "You cant use more GPUs than you have."
        # A selected single GPU may be cuda:N rather than cuda:0.  Wrapping
        # it in DataParallel(device_ids=[0]) moves the replica back to cuda:0.
        if num_GPU > 1:
...
def generate_missing_edges(solution: torch.Tensor, edge_index: torch.Tensor, edge_miss_rate: float):
    if edge_miss_rate <= 0:
        return solution, edge_index
    num_edges = edge_index.shape[1]
    num_missing = int(num_edges * edge_miss_rate)
    if num_missing <= 0:
        return solution, edge_index
    keep_mask = torch.ones(num_edges, dtype=torch.bool, device=edge_index.device)
    remove_indices = torch.randperm(num_edges, device=edge_index.device)[:num_missing]
    keep_mask[remove_indices] = False
    return solution, edge_index[:, keep_mask]


def sample_observed_dynamics(dynamics, edge_index, ob_node_rate):
    if ob_node_rate >= 1:
        return dynamics, edge_index, torch.arange(dynamics.shape[1], device=dynamics.device)

    T, N, F_dim = dynamics.shape
    num_observed = max(1, int(N * ob_node_rate))
    observed_nodes = torch.sort(torch.randperm(N, device=dynamics.device)[:num_observed]).values

    node_map = torch.full((N,), -1, dtype=torch.long, device=dynamics.device)
    node_map[observed_nodes] = torch.arange(num_observed, device=dynamics.device)

    edge_index_device = edge_index.to(dynamics.device)
    edge_mask = (node_map[edge_index_device[0]] >= 0) & (node_map[edge_index_device[1]] >= 0)
    observed_edge_index = node_map[edge_index_device[:, edge_mask]]
    observed_dynamics = dynamics[:, observed_nodes, :]
    return observed_dynamics, observed_edge_index.to(edge_index.device), observed_nodes


def generate_rpg(n, d=8, sparsity=0.5, scale=1.0):
    x = np.random.randn(n, d) * scale
    p = np.exp(-np.linalg.norm(x[:, None] - x, axis=2) ** 2) * sparsity
    adjacency = np.random.rand(n, n) < p
    np.fill_diagonal(adjacency, 0)
    return ensure_no_isolated_nodes(nx.from_numpy_array(adjacency))


def generate_dscm(n, exponent=2):
    in_degrees = np.random.zipf(exponent, size=n)
    out_degrees = np.random.zipf(exponent, size=n)
    diff = int(in_degrees.sum() - out_degrees.sum())
    if diff > 0:
        out_degrees[np.random.randint(n)] += diff
    elif diff < 0:
        in_degrees[np.random.randint(n)] += abs(diff)
    return nx.directed_configuration_model(in_degrees, out_degrees, create_using=nx.DiGraph)


def ensure_no_isolated_nodes(G):
    """Attach isolated nodes with a nearest/random fallback so mask discovery has samples."""
    ...
    for node in list(nx.isolates(G)):
        ...
        if node in pos and all(candidate in pos for candidate in candidates):
            target = min(
                candidates,
                key=lambda candidate: np.linalg.norm(np.asarray(pos[node]) - np.asarray(pos[candidate])),
            )
        else:
...
def generate_rgm(n, radius=0.05):
    return ensure_no_isolated_nodes(nx.random_geometric_graph(n, radius))


def generate_sbm(n, k=3, p_in=0.05, p_out=0.05, sparsity_factor=0.1):
    sizes = [n // k] * k
    sizes[-1] += n - sum(sizes)
    probs = np.full((k, k), p_out)
    np.fill_diagonal(probs, p_in)
    probs *= sparsity_factor
    return ensure_no_isolated_nodes(nx.stochastic_block_model(sizes, probs, seed=42))
...
def fun_lib(x, poly_p, poly_n, device, activate=False, mask=None, names=False, basis_variant="legacy"):
    ...
    if basis_variant == "trig_exp_v2":
        for d in range(dim):
            generators.extend([
                lambda x, d=d: torch.exp(torch.clamp(x[:, [d]], -10.0, 10.0)),
                lambda x, d=d: x[:, [d]] * torch.exp(torch.clamp(x[:, [d]], -10.0, 10.0)),
            ])
            name_list.extend([
                f"exp(x{d+1})",
                f"x{d+1}*exp(x{d+1})",
    ...
    if activate:
        ...
        if basis_variant == "trig":
            for d in range(dim):
                ...
                name_list.extend([f"sin(x{d+1})", f"cos(x{d+1})"])
        elif basis_variant == "trig_exp_v2":
            for d in range(dim):
                generators.extend([
                    lambda x, d=d: torch.sin(x[:, [d]]),
                    lambda x, d=d: torch.cos(x[:, [d]])
...
def coupled_fun_lib(x_i, x_j, poly_p, poly_n, device, activate=False, mask=None, names=False, basis_variant="legacy"):
    ...
    if activate:
        if basis_variant == "trig":
            generators.extend([
                lambda x, y: torch.sin(x),
                lambda x, y: torch.cos(x),
                lambda x, y: torch.sin(y + x),
                lambda x, y: torch.cos(y + x),
                lambda x, y: y * torch.sin(y),
                lambda x, y: y * torch.cos(y),
                lambda x, y: x * torch.sin(y - x),
                lambda x, y: x * torch.cos(y - x),
                lambda x, y: x * torch.sin(y * x),
                lambda x, y: x * torch.cos(y * x),
                lambda x, y: (y - x) * torch.sin(y),
                lambda x, y: (y - x) * torch.cos(y)
            ])
            name_list.extend([
                "sin(x_i)",
                "cos(x_i)",
            ...
            # Keep the non-multiplicative trigonometric extensions only.
            # The six terms removed in v3 are deliberately absent:
            # x_i*sin/cos(x_j-x_i), x_i*sin/cos(x_j*x_i), and
            # (x_j-x_i)*sin/cos(x_j).
            generators.extend([
                lambda x, y: torch.sin(x),
                lambda x, y: torch.cos(x),
                ...
                lambda x, y: y * torch.sin(y),
                lambda x, y: y * torch.cos(y),
                # Retain the rational coupling family required by Gene/MM.
                # These terms were previously available only in the legacy
                # branch, so MM's x_j/(x_j+1) truth term was absent from the
                # trig_exp_v2 candidate library.
                lambda x, y: y / (y + 1),
...
def fun_lib_by_names(x, poly_p, poly_n, device, basis_names, activate=False, basis_variant="legacy"):
    ...
    missing = [name for name in basis_names if name not in name_to_idx]
    if missing:
        raise ValueError(f"E2 self basis terms not found in SIGN library: {missing[:5]}")
    selected_idx = [name_to_idx[name] for name in basis_names]
    full_mask = torch.zeros(len(all_names), dtype=torch.float32, device=device)
    full_mask[selected_idx] = 1.0
    lib = fun_lib(x, poly_p, poly_n, device, activate=activate, mask=full_mask, basis_variant=basis_variant)
    basis_set = set(basis_names)
    selected_full_order = [name for name in all_names if name in basis_set]
    if selected_full_order != basis_names:
        position = {name: i for i, name in enumerate(selected_full_order)}
        order = torch.tensor([position[name] for name in basis_names], dtype=torch.long, device=device)
        lib = lib[:, order]
    return lib


def coupled_fun_lib_by_names(x_i, x_j, poly_p, poly_n, device, basis_names, activate=False, basis_variant="legacy"):
    """Evaluate coupling terms in the exact order requested by E2 artifacts."""
    all_names = coupled_fun_lib(
        None,
        None,
        poly_p,
        poly_n,
    ...
    missing = [name for name in basis_names if name not in name_to_idx]
    if missing:
        raise ValueError(f"E2 coupling basis terms not found in SIGN library: {missing[:5]}")
    selected_idx = [name_to_idx[name] for name in basis_names]
    full_mask = torch.zeros(len(all_names), dtype=torch.float32, device=device)
    full_mask[selected_idx] = 1.0
    lib = coupled_fun_lib(x_i, x_j, poly_p, poly_n, device, activate=activate, mask=full_mask, basis_variant=basis_variant)
    basis_set = set(basis_names)
    selected_full_order = [name for name in all_names if name in basis_set]
    if selected_full_order != basis_names:
        position = {name: i for i, name in enumerate(selected_full_order)}
