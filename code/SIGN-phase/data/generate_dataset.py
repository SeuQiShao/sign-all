...
from model.utils import (
    get_edgeindex_4_edges,
    generate_dscm,
    generate_missing_edges,
    generate_rgm,
    generate_rpg,
    generate_sbm,
    partition_graph_pyg,
...
class DatasetGenerator:
    ...
    def generate_relations(self):
        ...
        if network_type == 'random':
            ...
            G = generate_rgm(n, radius=self.args.rgm_radius)
        elif network_type == 'sbm':
            G = generate_sbm(n, k=self.args.sbm_blocks, p_in=self.args.sbm_p_in, p_out=self.args.sbm_p_out)
            ...
            try:
                ...
                if self.args.partition_parts > 1:
                    sub_edge_indices, sub_num_nodes = partition_graph_pyg(edge_index, n, num_parts=self.args.partition_parts)
    ...
    def initialize_model(self, edge_index):
        ...
        if self.args.model_name == 'HeatDiffusion':
            ...
            if self.args.init_noise > 0:
                self.model.omega = torch.normal(
                    mean=0.3,
                    std=self.args.init_noise,
                    size=(self.node_num, 1),
                    device=self.args.device,
                ).abs()
            self.para = [self.model.k, self.model.omega]
        elif self.args.model_name == 'SIS':
            self.model = SIS(edge_index).to(self.args.device)
            self.para = [self.model.a]
        elif self.args.model_name in ('Gene', 'MM'):
            self.model = GeneDynamics(edge_index).to(self.args.device)
            self.para = [self.model.h, self.model.f, self.model.b, self.model.e]
        elif self.args.model_name == 'Mutual':
            self.model = MutualDynamics(edge_index).to(self.args.device)
            self.para = [self.model.b, self.model.c, self.model.d, self.model.e, self.model.h, self.model.k]
        elif self.args.model_name == 'FHN':
        ...
        elif self.args.model_name == 'Chua':
            self.model = ChuaDynamics(edge_index).to(self.args.device)
            self.para = [
                self.model.alpha,
                self.model.beta,
                self.model.m0,
    ...
    def add_gaussian_noise_torch(self, x, snr_db):
        if snr_db <= 0:
            return x
    ...
    def generate_batch_sample(self):
        ...
        for _ in tqdm(range(self.args.sample_num)):
            ...
            if self.args.ob_node_rate < 1:
                solution, edge_index, observed_nodes = sample_observed_dynamics(
...
def get_args():
    ...
    parser.add_argument(
        '--network', type=str,
        choices=['random', 'power_law', 'small_world', 'from_file', 'rpg', 'dscm', 'rgm', 'sbm'],
        default='small_world',
    ...
    parser.add_argument('--file_path', type=str, default='../../networks/human-gene/bio-human-gene2.edges', help='file path')

    parser.add_argument('--undirected', type=bool, default=False, help='generate undirected graph')
    parser.add_argument('--partition_parts', type=int, default=1, help='Partition from_file graphs and use the first part when > 1')
    parser.add_argument('--rgm_radius', type=float, default=0.05, help='Radius for random geometric graph smoke tests')
    parser.add_argument('--sbm_blocks', type=int, default=3, help='Number of blocks for SBM smoke tests')
    ...
    parser.add_argument('--obnoise', type=float, default=0, help='Add Gaussian observation noise at this SNR in dB; 0 disables it')
    parser.add_argument('--miss_edge_rate', type=float, default=0, help='Fraction of edges removed before inference')
    parser.add_argument('--ob_node_rate', type=float, default=1, help='Fraction of observed nodes kept before inference')
    parser.add_argument('--init_noise', type=float, default=0, help='Kuramoto natural-frequency std for heterogeneity experiments')
    parser.add_argument('--stre', type=float, default=0.1, help='FHN coupling strength')
