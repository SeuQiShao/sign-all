...
class MutualDynamics(MessagePassing):
    ...
    def __init__(self, edge_index, edge_attr = None, aggr = 'sum'):
        ...
        if edge_attr is not None:
            ...
            self.edge_attr = torch.ones(edge_index.shape[1], 1, device=edge_index.device)
        self.c = 1
        self.d = 5
        self.b = 0.1
        self.k = 5
...
class ChuaDynamics(MessagePassing):
    """
    Network-coupled Chua circuit used for basis-mismatch smoke tests.

    dx/dt = alpha * (y - x - h(x)) + coupling
    dy/dt = x - y + z
    dz/dt = -beta * y
    h(x) = m1*x + 0.5*(m0-m1)*(|x+1|-|x-1|)
    """
    def __init__(self, edge_index, edge_attr=None, aggr='add'):
        super(ChuaDynamics, self).__init__()
        self.aggr = aggr
        self.edge_index = edge_index
        if edge_attr is not None:
            self.edge_attr = edge_attr
    ...
    def update(self, aggr_out, x):
        h_x = self.m1 * x[:, [0]] + 0.5 * (self.m0 - self.m1) * (
            torch.abs(x[:, [0]] + 1) - torch.abs(x[:, [0]] - 1)
        )
        x_d1 = self.alpha * (x[:, [1]] - x[:, [0]] - h_x)
        x_d2 = x[:, [0]] - x[:, [1]] + x[:, [2]]
        x_d3 = -self.beta * x[:, [1]]
        return aggr_out + torch.cat([x_d1, x_d2, x_d3], 1)
